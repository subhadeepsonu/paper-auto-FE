...
export default function CoordinatorCourses() {
    ...
    return (
        <div className="min-h-screen w-full  bg-gray-100 flex justify-center items-start pt-20 ">
            <div className="grid grid-cols-4 w-full gap-4 px-4">
                {QueryCourse.data.map((course: Course) => {
                    return <GeneralCoursesCard coordinator={course.coordinator.name} moderator={course.moderator.name} no_of_papers={course._count.papers} Id={course.id} courseCode={course.code} key={course.id} courseName={course.name} />
                })}
            </div>
        </div>
    );
